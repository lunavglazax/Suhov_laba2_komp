...
TButton btn[] = {
    {"kartinka", {0,0, 150,0, 150,60, 0,60}, FALSE},
    {"ydalit'", {0,80, 150,80, 150,140, 0,140}, FALSE},
    {"vixod", {0,160, 150,160, 150,220, 0,220}, FALSE}
    };
    ...
    switch (uMsg)
    {
        ...
        case WM_LBUTTONDOWN:
            ...
            if (PointInButton(LOWORD(lParam), HIWORD(lParam), btn[i])){
                printf("%s\n", btn[i].name);
                if (strcmp(btn[i].name, "kartinka") == 0)
                visible = TRUE;
                            if (strcmp(btn[i].name, "ydalit'") == 0)
                visible = FALSE;
                if (strcmp(btn[i].name, "vixod") == 0)
                    PostQuitMessage(0);
            }
            break;
